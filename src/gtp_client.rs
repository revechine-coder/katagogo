...
impl GtpClient {
    ...
    pub async fn close(&mut self) -> crate::error::Result<()> {
        if let Some(ref mut process) = self.process {
            ...
            let _ = tokio::time::timeout(std::time::Duration::from_secs(3), process.wait()).await;
        }
        self.process = None;
        Ok(())
    }
    ...
    pub async fn command(
        ...
        let stdin = process.stdin.as_mut().ok_or(GoCoreError::PipeClosed)?;
        let stdout = process.stdout.as_mut().ok_or(GoCoreError::PipeClosed)?;
