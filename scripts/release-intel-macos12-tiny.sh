...
RELEASE_DIR="${ROOT_DIR}/releases/intel-macos12-tiny"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/katagogo-intel-tiny.XXXXXX")"
ARCHIVE_PATH="${TMP_DIR}/KataGoGo-Intel-macOS12-tiny.xcarchive"
APP_PATH="${ARCHIVE_PATH}/Products/Applications/KataGoGo.app"
ZIP_PATH="${RELEASE_DIR}/KataGoGo-Intel-macOS12-tiny.app.zip"
...
TINY_MODEL="lionffen_b24c64.bin.gz"
MEDIUM_MODEL="kata1-b18c384nbt.bin.gz"
...
if [ ! -f "${ROOT_DIR}/kata-engine/${TINY_MODEL}" ]; then
  echo "error: missing tiny model: ${ROOT_DIR}/kata-engine/${TINY_MODEL}" >&2
  exit 1
fi
...
echo "==> Archiving Intel macOS 12 tiny-model app"
cd "${ROOT_DIR}"
...
xcodebuild -quiet \
  ...
  -derivedDataPath /tmp/KataGoGoIntelTinyDerivedData \
  -archivePath "${ARCHIVE_PATH}" \
  archive ARCHS=x86_64 ONLY_ACTIVE_ARCH=NO MACOSX_DEPLOYMENT_TARGET=12.0 ENABLE_USER_SCRIPT_SANDBOXING=NO

echo "==> Replacing medium model with tiny model"
rm -f "${APP_PATH}/Contents/Resources/kata-engine/${MEDIUM_MODEL}"
ditto "${ROOT_DIR}/kata-engine/${TINY_MODEL}" \
  "${APP_PATH}/Contents/Resources/kata-engine/${TINY_MODEL}"
chmod 644 "${APP_PATH}/Contents/Resources/kata-engine/${TINY_MODEL}"
...
echo "==> Verifying Intel tiny archive"
...
test -f "${APP_PATH}/Contents/Resources/kata-engine/${TINY_MODEL}"
test ! -f "${APP_PATH}/Contents/Resources/kata-engine/${MEDIUM_MODEL}"
codesign --verify --deep --strict --verbose=2 "${APP_PATH}"

echo "==> Smoke testing Intel tiny engine under Rosetta"
arch -x86_64 "${APP_PATH}/Contents/Resources/kata-engine/katago-eigen" gtp \
  ...
  -model "${APP_PATH}/Contents/Resources/kata-engine/${TINY_MODEL}" \
...
# KataGoGo Intel macOS 12 tiny-model packaged build

Artifact:

- \`KataGoGo-Intel-macOS12-tiny.app.zip\`
...
- Bundled tiny engine reached \`GTP ready\` under \`arch -x86_64\`
